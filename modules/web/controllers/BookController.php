...
use app\models\book\Book_cat;
use app\models\book\Book_stock_change_log;
...
class BookController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        $list = $query->orderBy([ 'id' => SORT_DESC ])
            ...
            ->all( );
        $cat_mapping = Book_cat::find()->orderBy([ 'id' => SORT_DESC ])->indexBy("id")->all();
    ...
    public function actionSet(){
        if( \Yii::$app->request->isGet ) {
            ...
            $cat_list = Book_cat::find()->orderBy([ 'id' => SORT_DESC ])->all();
        ...
        if( $model_book->save( 0 ) ){
            BookService::setStockChangeLog( $model_book->id,( $model_book->stock - $before_stock ) );//库存变更量unit用新添加的库存量减去之前的库存量
        }
    ...
    {
        $id = intval( $this->get("id", 0) );   //intval — 获取变量的整数值
        $reback_url = UrlService::buildWebUrl("/book/index");
        if( !$id ){
            return $this->redirect( $reback_url );  //如果没有id值就跳转到图书首页
        }

        $info = Book::find()->where([ 'id' => $id ])->one();
        if( !$info ){
            return $this->redirect( $reback_url );  //如果没有数据也跳到图书首页
        }

        //库存变更历史
        $stock_change_list = Book_stock_change_log::find()->where([ 'book_id' => $id ])
    ...
    {
        $image_list=Images::find()->orderBy(['id'=>SORT_DESC])->all();
        return $this->render('images',['image_list'=>$image_list]);
    }
    /*
        图书分类列表
     */
    ...
    public function actionCat_set(){
        if( \Yii::$app->request->isGet ){
            ...
            $info = [];
            if( $id ){
                $info = Book_cat::find()->where([ 'id' => $id ])->one();
            }
        ...
        $has_in = Book_cat::find()->where([ 'name' => $name ])->andWhere([ '!=','id',$id ])->count();
        if( $has_in ){
            return $this->renderJSON( [] , "该分类名称已存在，请换一个试试~~" ,-1);
        }

        $cat_info = Book_cat::find()->where([ 'id' => $id ])->one();
        if( $cat_info ){
            $model_book_cat = $cat_info;
        }else{
            $model_book_cat = new Book_cat();
